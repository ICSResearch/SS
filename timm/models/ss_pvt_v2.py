...
class Attention(nn.Module):
    def __init__(
        ...
        self.kv = nn.Linear(dim, hidden_dim * 2, bias=qkv_bias)
    ...
    def __init__(
        ...
        for i in range(num_stages):
            ...
            self.stages.append(PyramidVisionTransformerStage(
                ...
                mlp_ratios=mlp_ratios[start:start + depths[i]],
                splits=splits[start:start + depths[i]],
                pos_scales=pos_scales[start:start + depths[i]],
                act_layers=act_layers[start:start + depths[i]],
                attn_ratios=attn_ratios[start:start + depths[i]],
                attn_splits=attn_splits[start:start + depths[i]]
            ))
...
@register_model
def ss_pvt_v2_b0(pretrained=False, **kwargs):
    model_kwargs = dict(
        depths=(2, 2, 2, 2), embed_dims=(32, 64, 160, 256), num_heads=(1, 2, 5, 8),
        norm_layer=partial(nn.LayerNorm, eps=1e-6),
        mlp_ratios=[3.] * 1 + [3.5] * 1 + [4.] * 6,
        drop_path_rates=[x.item() for x in torch.linspace(0, 0.1, 8)],
        splits=[0.] * 7 + [0.5] * 1,
        act_layers=[nn.ReLU] * 2 + [nn.GELU] * 6,
        attn_ratios=[0.5] + [0.75] + [1.] * 6,
        pos_scales=[0.] * 3 + [x.item() for x in torch.linspace(0., 2, 3)] + [2.] * 2,
        **kwargs)
    return _create_pvt2('ss_pvt_v2_b0', pretrained=pretrained, **model_kwargs)
