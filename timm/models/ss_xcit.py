...
from .vision_transformer_eff import _cfg, Mlp
...
class XCA(nn.Module):
    ...
    def forward(self, x):
        ...
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, self.hidden_dim // self.num_heads).permute(2, 0, 3, 4, 1)
        ...
        x = (attn @ v).permute(0, 3, 1, 2).reshape(B, N, self.hidden_dim)
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            XCABlock(
                ...
                drop_path=drop_path_rates[i], mlp_ratio=mlp_ratios[i],
                split=splits[i], pos_scale=pos_scales[i],
                act_layer=act_layers[i],
                attn_split=attn_splits[i], attn_ratio=attn_ratios[i])
...
def ss_xcit_tiny_24_p16_224(pretrained=False, **kwargs):
    ...
    model = _create_xcit('ss_xcit_tiny_24_p16_224', pretrained=pretrained, **model_kwargs)
    return model
