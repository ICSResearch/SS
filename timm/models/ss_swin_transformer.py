...
from .vision_transformer_eff import checkpoint_filter_fn, get_init_weights_vit
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, num_heads, head_dim=None, window_size=7, qkv_bias=True,
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attn = WindowAttention(
            ...
            attn_ratio=attn_ratio, split=attn_split,
        )
...
class BasicLayer(nn.Module):
    ...
    def __init__(
            ...
            drop_path_rates=(0.,) * 24,
            splits=(0.,) * 24,
            pos_scales=(0,) * 24,
            act_layers=(nn.GELU,) * 24,
            attn_ratios=(1.,) * 24,
            attn_splits=(0.,) * 24,
            mlp_ratios=(4.,) * 24,
    ):
        ...
        self.blocks = nn.Sequential(*[
            SwinTransformerBlock(
                ...
                pos_scale=pos_scales[i], attn_ratio=attn_ratios[i],
                act_layer=act_layers[i], attn_split=attn_splits[i],
                split=splits[i]
            )
    ...
    def __init__(
        ...
        for i in range(self.num_layers):
            ...
            layers += [BasicLayer(
                ...
                drop_path_rates=drop_path_rates[start:start+depths[i]],
                norm_layer=norm_layer,
                downsample=PatchMerging if (i < self.num_layers - 1) else None,
                mlp_ratios=mlp_ratios[start:start+depths[i]], splits=splits[start:start+depths[i]],
                pos_scales=pos_scales[start:start+depths[i]],
                act_layers=act_layers[start:start+depths[i]],
                attn_ratios=attn_ratios[start:start+depths[i]], attn_splits=attn_splits[start:start+depths[i]]
            )]
...
@register_model
def ss_swin_base_patch4_window7_224(pretrained=False, **kwargs):
    ...
    model_kwargs = dict(
        ...
        depths=(2, 2, 18, 2),
        num_heads=(4, 8, 16, 32),
        mlp_ratios=[3.]*2 + [3.5]*2 + [4.] * 20,
        drop_path_rates=[x.item() for x in torch.linspace(0, 0.5, 24)],
        splits=[0.] * 20 + [0.5] * 4,
        act_layers=[nn.ReLU] * 4 + [nn.GELU] * 20,
        pos_scales=[0.] * 8 + [x.item() for x in torch.linspace(0., 2, 8)] + [2.] * 8,
        attn_ratios=[0.5]*2 + [0.75]*2 + [1.] * 20,
        attn_splits=[0.] * 20 + [0.25] * 4,
        **kwargs)
    return _create_swin_transformer('ss_swin_base_patch4_window7_224', pretrained=pretrained, **model_kwargs)


@register_model
def ss_swin_base_patch4_window7_384(pretrained=False, **kwargs):
    """ Swin-B @ 224x224, pretrained ImageNet-22k, fine tune 1k
    """
    model_kwargs = dict(
        patch_size=4, window_size=12,
        img_size=384,
        embed_dim=128,
        depths=(2, 2, 18, 2),
        num_heads=(4, 8, 16, 32),
        ...
        attn_splits=[0.] * 20 + [0.25] * 4,
        **kwargs)
    return _create_swin_transformer('ss_swin_base_patch4_window7_384'
