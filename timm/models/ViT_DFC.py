...
Hacked together by / Copyright 2021 Ross WightmanG
"""
import math
import logging
...
from .layers.DFC import MlpD
# from .layers.mlp_AS import MlpRV2 as MlpRF
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        self.mlp = MlpD(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop,
                        ratio=2)
...
class VisionTransformer(nn.Module):
    ...
    def init_weights(self, mode=''):
        ...
        trunc_normal_(self.pos_embed, std=.01)
        if self.dist_token is not None:
            trunc_normal_(self.dist_token, std=.01)
        if mode.startswith('jax'):
            # leave cls token as zeros to match jax impl
            named_apply(partial(_init_vit_weights, head_bias=head_bias, jax_impl=True), self)
        else:
            trunc_normal_(self.cls_token, std=.01)
...
@register_model
def dfc_deit_tiny_patch16_224(pretrained=False, **kwargs):
...
@register_model
def dfc_deit_small_patch16_224(pretrained=False, **kwargs):
...
@register_model
def dfc_deit_base_patch16_224(pretrained=False, **kwargs):
